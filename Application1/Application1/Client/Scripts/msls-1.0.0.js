...
(function () {
    ...
    var currentLayout = [],
        ...
        rootSelector = "body > div:not(.msls-layout-ignore)";
    ...
    msls_updateLayoutImmediately = function ($rootNodes) {
    ...
    function _invokeLayout() {
        var tempIsolatedNodes = isolatedNodeList,
            tempRequest = fullUpdateRequested,
            tempNonIsolatedNodes = nonIsolatedNodeList;

        _clearQueue(true);
        if (tempRequest) {
            updateLayoutCore($(rootSelector), false);
        } else if (!!tempNonIsolatedNodes) {
            updateLayoutCore(tempNonIsolatedNodes, false);
        }
        if (!!tempIsolatedNodes) {
            updateLayoutCore(tempIsolatedNodes, true);
        }
    }
    ...
    function _queue() {
        function _handleCallback() {
            if (requestReceived) {
                ...
                queueHandle = setTimeout(_handleCallback, 1);
                return;
            }
    ...
    msls_updateLayout =
    function updateLayout($rootNodes) {
    ...
    function updateLayoutCore($rootNodes, isolated) {
        ...
        try {
            msls_notify(msls_layout_updatingNotification);

            $rootNodes = $rootNodes.filter(":visible");

            updateHeight($rootNodes, isolated);
            updateWidth($rootNodes, isolated);
            msls_notify(msls_layout_updatedNotification);
        } finally {
            layoutUpdating = false;
        }
    }
    ...
    function updateHeight(rootNodes, isolated) {
        ...
        var vStretchElements = $(".msls-vstretch", rootNodes).add(rootNodes.filter(".msls-vstretch"));
        if (!isolated){
            vStretchElements = vStretchElements.not(".msls-layout-ignore-children .msls-vstretch");
        }
        ...
        $.each(vStretchElements, function () {
            ...
            $.each(getVAutoNodes(parentNode), function () {
                totalSiblingsHeight += contentHeight(this, true);
            });
            ...
            $.each($(parentNode).children(".msls-vstretch").filter(":visible"), function () {
                weightedTotal += getWeight(this);
            });

            arrangeRows(parentNode, totalAvailableHeight, weightedTotal);
            $(parentNode).data("msls-rows-updated", true);
        });
    }

    function arrangeRows(parentNode, totalAvailableHeight, weightedTotal) {
        ...
        $.each(parentNode.children, function () {
            ...
            if (!node.hasClass("msls-vstretch") || node.filter(":visible").length === 0) {
                return;
            }
    ...
    function updateWidth(rootNodes, isolated) {

        var columnLayouts = $(".msls-columns-layout", rootNodes).add(rootNodes.filter(".msls-columns-layout"));
        if (!isolated){
        ...
        $.each(columnLayouts, function () {
            ...
            $.each(this.children, function () {
                if ($(this).hasClass("msls-hstretch")) {
        ...
        $.each(columnLayouts, function () {
            var node = $(this),
                availableWidth = getAvailableChildWidth(this),
                newAvailableWidth;

            arrangeColumns(this, availableWidth);

            newAvailableWidth = getAvailableChildWidth(this);
            if (availableWidth !== newAvailableWidth) {
                arrangeColumns(this, newAvailableWidth);
            }
        });
    }

    function arrangeColumns(parentNode, parentWidth) {
        ...
        $.each(parentNode.children, function () {
            ...
            if (node.hasClass("msls-hauto")) {
                totalSiblingWidth += contentWidth(this, true);
        ...
        columnDetails = [];
        hasStretch = false;
        $.each(parentNode.children, function (index) {
            var node = $(this),
                weight = getWeight(this),
                computedStyle = window.getComputedStyle(this),
                margin = parseFloat(computedStyle.marginLeft) + parseFloat(computedStyle.marginRight),
                requiredWidth,
                isVisible = node.filter(":visible").length === 1,
                isAuto = node.hasClass("msls-hauto") && isVisible,
                isStretch = node.hasClass("msls-hstretch") && isVisible,
                isEmpty = node.hasClass("msls-hempty"),
                minWidth = 0;

            if (isAuto) {
                requiredWidth = contentWidth(this, true);
            } else if (isStretch) {
            ...
            if (isAuto || isStretch) {
                columnDetails.push({
                    element: this,
                    isAuto: isAuto,
                    totalWidth: requiredWidth + margin,
                    weight: weight
                });
            }
        });
        ...
        if (hasStretch) {
            if (overflowColumns) {
                ...
                $.each(rowData, function () {
                    fitColumnsIntoRow(this, parentWidth, overflowColumns);
                });
            } else {
                $.each(columnDetails, function () {
                    if (!this.isAuto) {
                        setWidth(this.element, this.totalWidth, overflowColumns);
                    }
                });
            }
        }
        if ($(parentNode).hasClass("msls-dynamic-padding")) {
            setColumnClasses(parentNode);
        }
    }
    ...
    function fitColumnsIntoRow(columnsToFit, availableWidth, isOverflowing) {
        ...
        $.each(columnsToFit, function (columnIndex) {
            ...
            if (isLast) {
                node.next(".msls-hempty").addClass("msls-clear");
            } else {
                node.next(".msls-hempty").removeClass("msls-clear");
            }
        });
        ...
        $.each(hstretchColumns, function () {
            ...
            setWidth(this.element, width, isOverflowing);
        });
    }

    function setColumnClasses(parentNode) {
        var children = [],
            rowIndexTotal = -1,
            childrenData = [],
            isOverflowing = false;
        ...
        $.each(childrenData, function (index) {
            var isFirst = index === 0,
                isLast = index === childrenData.length - 1,
                prevOffset = isFirst ? -1 : childrenData[index - 1].offsetTop,
                nextOffset = isLast ? -1 : childrenData[index + 1].offsetTop,
                currentOffset = this.offsetTop,
                needFirstColumnAttribute = isFirst || currentOffset !== prevOffset,
                needLastColumnAttribute = isLast || currentOffset !== nextOffset,
                needMiddleColumnAttribute = !needFirstColumnAttribute && !needLastColumnAttribute;

            if (currentOffset !== prevOffset) {
                rowIndexTotal++;
            }
            this.rowIndex = rowIndexTotal;

            if (needFirstColumnAttribute && needLastColumnAttribute) {
                this.addClassString = " msls-first-column msls-last-column";
                this.removeClassString = " msls-column";
            } else {
                if (needFirstColumnAttribute) {
                    this.addClassString = " msls-first-column";
                    this.removeClassString = " msls-last-column msls-column";
                } else if (needLastColumnAttribute) {
                    this.addClassString = " msls-last-column";
                    this.removeClassString = " msls-first-column msls-column";
                } else {
                    this.addClassString = " msls-column";
                    this.removeClassString = " msls-first-column msls-last-column";
                }
            }
        });
        ...
        $.each(childrenData, function () {
            ...
            if (rowIndexTotal === 0) {
                this.removeClassString += " msls-first-row msls-row msls-last-row";
                return;
            }

            isFirst = this.rowIndex === 0;
            isLast = this.rowIndex === rowIndexTotal;
            isMiddle = !isFirst && !isLast;

            if (isFirst && isLast) {
                this.addClassString += " msls-first-row msls-last-row";
                this.removeClassString += " msls-row";
            } else {
                if (isFirst) {
                    this.addClassString += " msls-first-row";
                    this.removeClassString += " msls-row msls-last-row";
                } else if (isLast) {
                    this.addClassString += " msls-last-row";
                    this.removeClassString += " msls-first-row msls-row";
                } else {
                    this.addClassString += " msls-row";
                    this.removeClassString += " msls-first-row msls-last-row";
                }
            }
        });
        $.each(childrenData, function (index) {
    ...
    function contentHeight(element, includeMargins) {
        var style = window.getComputedStyle(element),
            result = parseFloat(style.height) + parseFloat(style.borderTopWidth) + parseFloat(style.borderBottomWidth) + parseFloat(style.paddingTop) + parseFloat(style.paddingBottom);

        if (includeMargins) {
            result += parseFloat(style.marginTop) + parseFloat(style.marginBottom);
        }

        result = element.offsetHeight > result ? element.offsetHeight : result;
        return isNaN(result) ? 0 : Math.ceil(result);
    }

    function contentWidth(element, includeMargins) {

        var style = window.getComputedStyle(element),
            result = parseFloat(style.width) + parseFloat(style.borderLeftWidth) + parseFloat(style.borderRightWidth) + parseFloat(style.paddingLeft) + parseFloat(style.paddingRight);

        result = element.offsetWidth > result ? element.offsetWidth : result;

        if (includeMargins) {
            result += parseFloat(style.marginLeft) + parseFloat(style.marginRight);
        }
    ...
    function getAvailableChildWidth(element) {
        ...
        if (element.clientWidth !== 0) {
            result = element.clientWidth < result ? element.clientWidth : result;
        }
        result -= parseFloat(style.borderLeftWidth) + parseFloat(style.borderRightWidth) + parseFloat(style.paddingLeft) + parseFloat(style.paddingRight);
    ...
    function getAvailableChildHeight(element) {
        ...
        var style = window.getComputedStyle(element),
            result = parseFloat(style.height);

        if (element.clientHeight !== 0) {
            result = element.clientHeight < result ? element.clientHeight : result;
        }

        result -= parseFloat(style.borderTopWidth) + parseFloat(style.borderBottomWidth) + parseFloat(style.paddingTop) + parseFloat(style.paddingBottom);

        return isNaN(result) ? 0 : Math.floor(result);
    }

    function setWidth(element, width, isOverflowing) {
        ...
        element.style.width = resultingWidth + "px";

        if (!isOverflowing && minWidth > resultingWidth) {
            element.style.minWidth = resultingWidth + "px";
    ...
    function findNodeDepth(element) {
        var depth = 0;
    ...
    function findAffectedSubtree(element) {
        ...
        var $leafNode = $(element),
            $verticalClosest = $leafNode.closest(".msls-vstretch, .msls-fixed-height"),
            $horizontalClosest = $leafNode.closest(".msls-hstretch, .msls-fixed-width");
        if (!$verticalClosest.length && !$horizontalClosest.length) {
            return null;
        }

        if (!$verticalClosest.length) {
            return $horizontalClosest;
        }
        if (!$horizontalClosest.length) {
            return $verticalClosest;
        }

        var verticalDepth = findNodeDepth($verticalClosest[0]),
            horizontalDepth = findNodeDepth($horizontalClosest[0]);

        return (verticalDepth < horizontalDepth) ? $verticalClosest : $horizontalClosest;
    }
    ...
    $(document).ready(function () {
        $("body").bind("updatelayout", function (e) {
